...
function VoiceSelector() {
  ...
  const currentVoice = selectedVoice
    ...
    : hasMissingSelectedVoice
      ? { id: voiceId, name: "Unavailable voice", category: null }
      : voices[0];
  ...
  return (
    <Field>
      ...
      >
        <SelectTrigger className="w-full h-auto rounded-lg bg-white gap-1 px-2 py-1">
          <SelectValue>
            {currentVoice && (
              <>
        ...
        <SelectContent>
          ...
          {systemVoices.length > 0 && (
            <SelectGroup>
              <SelectLabel>Built-in voice</SelectLabel>
              {systemVoices.map((v) => (
                <SelectItem key={v.id} value={v.id}>
                  <VoiceAvatar seed={v.id} name={v.name} />
                  <span className="truncate text-sm font-medium">
